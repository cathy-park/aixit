...
export function ToolCard({
  ...
  const hasCreds = Boolean(tool.credentialId || tool.credentialSecret);
  const accountProvider = tool.credentialProvider ?? "email";
  ...
  return (
    ...
    >
      ...
      {hasNote ? (
        <div className="mt-5 flex min-h-[52px] items-center rounded-2xl border border-orange-200/90 bg-[#fffbeb] px-4 py-3 text-sm font-medium leading-snug text-zinc-950">
          {tool.highlightNote}
        </div>
      ) : null}
          ...
          <div className="relative w-full max-w-md rounded-3xl bg-white p-6 shadow-2xl ring-1 ring-zinc-200">
            ...
            <div className="mt-5 space-y-4">
              <div>
                ...
                <div className="mt-1 font-mono text-sm font-semibold text-zinc-900">{tool.credentialId ?? "—"}</div>
              </div>
              <div>
                ...
                <div className="mt-1 flex items-center justify-between gap-3">
                  <span className="min-w-0 truncate font-mono text-sm font-semibold tracking-wide text-zinc-900">
                    {showSecret ? tool.credentialSecret ?? "—" : "●●●●●●●●●●"}
                  </span>
                  <button
                    type="button"
