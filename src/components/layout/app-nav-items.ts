import type { LucideIcon } from "lucide-react";
import {
  BookOpen,
  Calendar,
  FolderKanban,
  Home,
  Lightbulb,
  MessageSquare,
  Settings,
  StickyNote,
  Wrench,
  FileText,
} from "lucide-react";

export type AppNavItem = {
  id: string;
  href: string;
  label: string;
  icon: LucideIcon;
  match: (p: string) => boolean;
  /** true이면 설정에서 숨길 수 없음 */
  alwaysVisible?: boolean;
};

/** 사이드바·모바일 상단 메뉴 공통 */
export const APP_NAV_ITEMS: AppNavItem[] = [
  { id: "dashboard", href: "/dashboard", label: "홈", icon: Home, match: (p) => p === "/dashboard" },
  { id: "calendar", href: "/calendar", label: "캘린더", icon: Calendar, match: (p) => p.startsWith("/calendar") },
  {
    id: "projects",
    href: "/projects",
    label: "프로젝트",
    icon: FolderKanban,
    match: (p) => p === "/projects" || p.startsWith("/workspace") || p.startsWith("/recommendation"),
  },
  {
    id: "workflows",
    href: "/workflows",
    label: "워크플로우",
    icon: BookOpen,
    match: (p) => p.startsWith("/workflows") || p.startsWith("/workflow/"),
  },
  { id: "tools", href: "/tools", label: "도구 창고", icon: Wrench, match: (p) => p.startsWith("/tools") },
  {
    id: "inspiration",
    href: "/inspiration",
    label: "영감 창고",
    icon: Lightbulb,
    match: (p) => p.startsWith("/inspiration"),
  },
  {
    id: "chatbots",
    href: "/chatbots",
    label: "챗봇 창고",
    icon: MessageSquare,
    match: (p) => p.startsWith("/chatbots"),
  },
  { id: "memos", href: "/memos", label: "메모", icon: StickyNote, match: (p) => p.startsWith("/memos") },
  { id: "minutes", href: "/minutes", label: "회의록", icon: FileText, match: (p) => p.startsWith("/minutes") },
  { id: "settings", href: "/settings", label: "설정", icon: Settings, match: (p) => p.startsWith("/settings"), alwaysVisible: true },
];
