...
import { stripBracketPlaceholderSubtitle } from "@/lib/memo-subtitle-placeholders";
...
export function dashboardWorkflowToPreview(
  ...
  /** 카드·검색용: 사용자가 워크스페이스에 적은 설명만 (메모 승격 시 `[대상]` 같은 힌트만 있으면 제거) */
  const subtitle = stripBracketPlaceholderSubtitle(typeof w.subtitle === "string" ? w.subtitle : "");
