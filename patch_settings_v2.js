...
const hooksStr = '  const { isVisible, toggle } = useNavVisibility();';
const newHooksStr = `  const { isVisible, toggle } = useNavVisibility();
...
// Change map
content = content.replace('{APP_NAV_ITEMS.map((item) => {', '{orderedItems.map((item) => {');
